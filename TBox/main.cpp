...
const uint32_t TO[256] = {0xa56363c6, 0x847c7cf8, 0x997777ee, 0x8d7b7bf6, 0x0df2f2ff, 0xbd6b6bd6, 0xb16f6fde, 0x54c5c591, 0x50303060, 0x03010102, 0xa96767ce, 0x7d2b2b56, 0x19fefee7, 0x62d7d7b5, 0xe6abab4d, 0x9a7676ec,
...
const uint32_t T1[256] = {0x6363c6a5, 0x7c7cf884, 0x7777ee99, 0x7b7bf68d, 0xf2f2ff0d, 0x6b6bd6bd, 0x6f6fdeb1, 0xc5c59154, 0x30306050, 0x01010203, 0x6767cea9, 0x2b2b567d, 0xfefee719, 0xd7d7b562, 0xabab4de6, 0x7676ec9a,
...
const uint32_t T2[256] = {0x63c6a563, 0x7cf8847c, 0x77ee9977, 0x7bf68d7b, 0xf2ff0df2, 0x6bd6bd6b, 0x6fdeb16f, 0xc59154c5, 0x30605030, 0x01020301, 0x67cea967, 0x2b567d2b, 0xfee719fe, 0xd7b562d7, 0xab4de6ab, 0x76ec9a76,
...
const uint32_t T3[256] = {0xc6a56363, 0xf8847c7c, 0xee997777, 0xf68d7b7b, 0xff0df2f2, 0xd6bd6b6b, 0xdeb16f6f, 0x9154c5c5, 0x60503030, 0x02030101, 0xcea96767, 0x567d2b2b, 0xe719fefe, 0xb562d7d7, 0x4de6abab, 0xec9a7676,
...
void generateT0 () {
    ...
    for (uint8_t i = 0; i <= 0xff; i++) {
        uint8_t tmp = SBOX[i];
        array[i] = 0;
        array[i] = ((uint32_t)(xtime(tmp))) ^ (((uint32_t)(tmp)) << 8) ^ (((uint32_t)(tmp)) << 16) ^ (((uint32_t)(xtime(tmp) ^ tmp)) << 24);
        if (i == 0xff)
            break;
    }
    for (int i = 0; i <= 255; i++) {
...
void generateT1 () {
    ...
    for (uint8_t i = 0; i <= 0xff; i++) {
        uint8_t tmp = SBOX[i];
        array[i] = 0;
        array[i] = ((uint32_t)(xtime(tmp) ^ tmp)) ^ (((uint32_t)(xtime(tmp))) << 8) ^ (((uint32_t)(tmp)) << 16) ^ (((uint32_t)((tmp))) << 24);
        if (i == 0xff)
            break;
    }
    for (int i = 0; i <= 255; i++) {
...
void generateT2 () {
    ...
    for (uint8_t i = 0; i <= 0xff; i++) {
        uint8_t tmp = SBOX[i];
        array[i] = 0;
        array[i] = ((uint32_t)(tmp)) ^ (((uint32_t)(xtime(tmp) ^ tmp)) << 8) ^ (((uint32_t)(xtime(tmp))) << 16) ^ (((uint32_t)((tmp))) << 24);
        if (i == 0xff)
            break;
    }
    for (int i = 0; i <= 255; i++) {
...
void generateT3 () {
    ...
    for (uint8_t i = 0; i <= 0xff; i++) {
        uint8_t tmp = SBOX[i];
        array[i] = 0;
        array[i] = ((uint32_t)(tmp)) ^ (((uint32_t)(tmp)) << 8) ^ (((uint32_t)(xtime(tmp) ^ tmp)) << 16) ^ (((uint32_t)(xtime(tmp))) << 24);
        if (i == 0xff)
            break;
    }
    for (int i = 0; i <= 255; i++) {
...
void magic (t_state s) {
    t_state state;

    state[0] = (TO[(s[0] & 0x000000ff)       ]) ^ (T1[(s[1] & 0x0000ff00) >> 8 ]) ^      //0->0, 1->1, 2->2, 3->3
               (T2[(s[2] & 0x00ff0000) >> 16 ]) ^ (T3[(s[3] & 0xff000000) >> 24]);

    state[1] = (TO[(s[1] & 0x000000ff)       ]) ^ (T1[(s[2] & 0x0000ff00) >> 8 ]) ^      //0->1, 1->2, 2->3, 3->0
               (T2[(s[3] & 0x00ff0000) >> 16 ]) ^ (T3[(s[0] & 0xff000000) >> 24]);

    state[2] = (TO[(s[2] & 0x000000ff)       ]) ^ (T1[(s[3] & 0x0000ff00) >> 8 ]) ^      //0->2, 1->3, 2->0, 3->1
               (T2[(s[0] & 0x00ff0000) >> 16 ]) ^ (T3[(s[1] & 0xff000000) >> 24]);

    state[3] = (TO[(s[3] & 0x000000ff)       ]) ^ (T1[(s[0] & 0x0000ff00) >> 8 ]) ^      //0->3, 1->0, 2->1, 3->2
               (T2[(s[1] & 0x00ff0000) >> 16 ]) ^ (T3[(s[2] & 0xff000000) >> 24]);
    for (int i = 0; i < 4; i++) {
        s[i] = state[i];
    }
}
